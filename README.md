...
В исходнике встречаются параллельные имена (`WorkCard`/`WorkItem`, `ON_REVIEW`/`UNDER_REVIEW`, `REWORK`/`NEEDS_REVISION`, `EXECUTOR`/`CONTRACTOR` и др.). Они сохранены и явно отмечены в `docs/00-scope-and-decisions.md`, чтобы информация не была потеряна.

Точные команды запуска и точные имена переменных окружения исходный документ не фиксирует полностью. Поэтому `.env.example` содержит рекомендуемый шаблон, а не выдуманный «официальный» контракт.

## Критерий готовности

Для каждого обязательного бизнес-требования должно быть понятно:
